"""Lollipop figures for factor-score neuromaps correlations."""
...
from gradient_lib.neuromaps_correlations import (
...
from gradient_lib.plots_bars_voxelwise import _BAR_FIGURE_FONT_RCPARAMS

from .config import DEFAULT_MAP_VARIANT, neuromaps_space_slug, parse_neuromaps_spaces
from .correlations import neuromaps_correlation_csv_path
...
def _prepare_panel_table(
    ...
    *,
    ...
    panel_df = panel_df.sort_values("abs_r", ascending=False)
    if top_k is not None:
        panel_df = panel_df.head(top_k)
    labels = [
        _short_label(
            annotation_display_label(
                row["source"],
    ...
    return labels, values, panel_df


def _add_significance_marks_vertical(ax: plt.Axes, panel_df: pd.DataFrame, x: np.ndarray) -> None:
    if "p_null" not in panel_df.columns:
        return
...
def _add_significance_marks_horizontal(ax: plt.Axes, panel_df: pd.DataFrame, y: np.ndarray) -> None:
    if "p_null" not in panel_df.columns:
        return
    for yi, (_, row) in enumerate(panel_df.iterrows()):
        p = row.get("p_null")
        if pd.notna(p) and float(p) < _P_SIG:
            x = float(row["pearson_r"])
            ax.text(
...
def _plot_lollipop_panel(
    ...
    *,
    ...
    top_k: int | None = NEUROMAPS_TOP_K,
) -> None:
    if panel_df.empty:
        ax.text(
            0.5,
    ...
    labels, values, panel_df = _prepare_panel_table(
        panel_df,
        description_lookup,
        metadata,
        top_k=top_k,
    ...
    _add_significance_marks_vertical(ax, panel_df, x)
...
def _plot_lollipop_panel_horizontal(
    ax: plt.Axes,
    panel_df: pd.DataFrame,
    *,
    title: str,
    ...
    """Horizontal lollipops for long annotation lists (all correlations)."""
    if panel_df.empty:
        ax.text(
            0.5,
            0.5,
    ...
    ax.hlines(y, 0.0, values, colors=_LOLLIPOP_COLOR, linewidth=_LOLLIPOP_LINEWIDTH, zorder=2)
    ax.scatter(
        values,
        y,
        s=_LOLLIPOP_MARKERSIZE * 0.75,
        c=_LOLLIPOP_COLOR,
        edgecolors=_LOLLIPOP_COLOR,
        linewidths=_LOLLIPOP_EDGEWIDTH,
        zorder=3,
    )
    _add_significance_marks_horizontal(ax, panel_df, y)

    ax.axvline(0.0, color="k", lw=0.6, alpha=0.45, zorder=0)
    ax.set_yticks(y)
    ax.set_yticklabels(labels, fontsize=_AXIS_TICK_FS - 2)
    ax.invert_yaxis()
    if xlim is not None:
        ax.set_xlim(xlim[0], xlim[1])
    else:
        xmax = max(0.05, float(np.nanmax(np.abs(values))) * 1.15)
        ax.set_xlim(-xmax, xmax)
    ax.set_title(title, fontsize=_AXIS_LABEL_FS, color=GRADIENT_AXIS_LABEL_COLOR)
    if show_xlabel:
        ax.set_xlabel(xlabel, fontsize=_AXIS_LABEL_FS, color=GRADIENT_AXIS_LABEL_COLOR)
    else:
        ax.set_xlabel("")
    ax.tick_params(axis="x", labelsize=_AXIS_TICK_FS, colors=GRADIENT_AXIS_LABEL_COLOR)
    ax.spines["top"].set_visible(False)
    ax.spines["right"].set_visible(False)


def plot_factor_neuromaps_lollipop_all(
    csv_path: Path,
    out_path: Path,
    factor_tag: str,
    *,
    map_variant: str = DEFAULT_MAP_VARIANT,
    space: str = NEUROMAPS_SPACE_MNI,
    metadata_csv: Path | None = None,
) -> Path:
    """One factor, one space: horizontal lollipops for all deduplicated correlations."""
    if not csv_path.is_file():
        raise FileNotFoundError(f"Missing neuromaps CSV: {csv_path}")
    ...
    row_title = _SPACE_ROW_TITLE.get(space, space)

    n_labels = len(dedupe_correlations_by_description_brief(sub, metadata))
    fig_w = 12.0
    fig_h = max(8.0, 0.38 * n_labels)
    ...
    with plt.rc_context(_BAR_FIGURE_FONT_RCPARAMS):
        ...
        _plot_lollipop_panel_horizontal(
            ax,
            sub,
            title=row_title,
            show_xlabel=True,
        ...
        fig.suptitle(
            f"{_factor_display_name(factor_tag)} — all neuromaps correlations "
            f"({row_title}; group-mean factor scores, {map_variant})",
...
def plot_neuromaps_lollipop_summary_for_space(
    ...
    *,
    ...
    """One row × N factor columns of top-|r| neuromaps lollipops for a single space."""
    ...
    with plt.rc_context(_BAR_FIGURE_FONT_RCPARAMS):
        ...
        fig.suptitle(
            f"{row_title} — group-mean factor scores ({map_variant}); "
            f"top {NEUROMAPS_TOP_K} |r| neuromaps annotations",
            fontsize=_AXIS_LABEL_FS + 2,
            color=GRADIENT_AXIS_LABEL_COLOR,
            y=0.98,
        )
...
def plot_neuromaps_lollipop_summary(
    ...
    *,
    ...
    with plt.rc_context(_BAR_FIGURE_FONT_RCPARAMS):
        ...
        fig.suptitle(
            f"Group-mean factor scores ({map_variant}) — top {NEUROMAPS_TOP_K} |r| neuromaps annotations",
            fontsize=_AXIS_LABEL_FS + 2,
            color=GRADIENT_AXIS_LABEL_COLOR,
            y=0.98,
        )
...
def plot_neuromaps_figures_from_output(
    ...
    *,
    ...
    csv_map = {
        f: neuromaps_correlation_csv_path(
            output_dir, f, map_variant=map_variant, cohort_tag=cohort_tag
        )
        for f in factors
    }

    for space in spaces:
        slug = neuromaps_space_slug(space)
        summary_out = (
            fig_dir
            / f"factor-score_{map_variant}_neuromaps_lollipop_{slug}_cohort-{cohort_tag}.png"
        )
        plot_neuromaps_lollipop_summary_for_space(
            csv_map,
            summary_out,
            space=space,
        ...
        saved.append(summary_out)

        for factor in factors:
            factor_out = (
                fig_dir
                / f"{factor}_factor-score_{map_variant}_neuromaps_lollipop_all_{slug}_cohort-{cohort_tag}.png"
            )
            plot_factor_neuromaps_lollipop_all(
                csv_map[factor],
                factor_out,
                factor,
                map_variant=map_variant,
