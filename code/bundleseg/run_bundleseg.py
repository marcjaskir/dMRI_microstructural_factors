...
in_tractograms = ff"{project_root()}/derivatives/trekker/{group}/{sub}/{sub}_space-ACPC_desc-preproc_trekker.trk"
in_config_file = f"{project_root()}/code/bundleseg/config/config_HCP1065_association_projection.json"
# in_config_file = f"{project_root()}/code/bundleseg/config/config_C_PO_L.json"
in_directory = f"{project_root()}/data/atlases/HCP1065/association_projection_bundleseg"

if group in ["penn_epilepsy", "penn_controls", "hcpaging"]:
    in_transfo = ff"{project_root()}/derivatives/acpc_mni_xfm/{group}/{sub}/{sub}_from-ACPC_to-MNI152NLin2009cAsym_AffineTransform.mat"
elif group == "hcpya":
    in_transfo = ff"{project_root()}/derivatives/acpc_mni_xfm/{group}/{sub}/{sub}_from-T1w_to-MNI152NLin2009cAsym_AffineTransform.mat"

out_dir = ff"{project_root()}/derivatives/bundleseg/{group}/{sub}"
if not os.path.exists(out_dir):
    os.makedirs(out_dir)

# Define directories for anatomical reference
qsiprep_dir = ff"{project_root()}/derivatives/qsiprep/{group}"
hcp_raw_dir = f"{project_root()}/data/hcpya/hcp1200/HCP1200"
